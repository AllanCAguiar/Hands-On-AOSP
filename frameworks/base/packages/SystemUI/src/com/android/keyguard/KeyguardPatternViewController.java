...
public class KeyguardPatternViewController
    ...
    protected KeyguardPatternViewController(KeyguardPatternView view,
        ...
        mDpm = mView.getContext().getSystemService(DevicePolicyManager.class);
    }
    ...
    protected void onViewAttached() {
        ...
        mRemainingAttemptsText = mView.findViewById(R.id.remaining_attempts_text);
    ...
    private void updateRemainingAttempts() {
        final int userId = KeyguardUpdateMonitor.getCurrentUser();
        if (mLockPatternUtils == null || userId == -1) {
            return;
        }

        int failedAttempts = mDpm.getCurrentFailedPasswordAttempts();
        
        int maxAttempts = LockPatternUtils.FAILED_ATTEMPTS_BEFORE_WIPE_GRACE;

        int remaining = maxAttempts - failedAttempts;

        if (failedAttempts > 0 && remaining > 0) {
            String text = getContext().getString(R.string.keyguard_remaining_attempts, remaining);
            mRemainingAttemptsText.setText(text);
